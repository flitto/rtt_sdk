...
extension ViewModel {
  ...
  private func loadTranslation(chatList: [TranslationEntity.CompositeChatItem], _ dstLangCode: String) async {
    await withTaskGroup(of: Void.self) { [weak self] group in
...
extension ViewModel {
  private func handleResponseChat(_ chatItem: RealTimeEntity.Chat.Response) async {
      ...
      case .update:
        let newArr = item.contentData.chatList.reduce(self) { current, next in
          ...
          if let idx = current.firstIndex(where: { $0.item.chatID == next.chatID }) {
            ...
            // 업데이트 대상이 현재 채팅 리스트에 포함이 안되어 있는 경우 (빈 값인 채팅을 수정한 경우)
